...
class Magecomp_Adminhtml_Block_System_Config_Form_Fieldset_Magecomp_Support
	extends Magecomp_Adminhtml_Block_System_Config_Form_Fieldset_Magecomp_Abstract
{
    ...
    public function render(Varien_Data_Form_Element_Abstract $element)
    {
		...
		$fields = array(
            ...
            array('type' => 'label', 'name' => 'send', 'after_element_html' => '<div class="right"><button type="button" class="scalable save" onclick="magecompSupport();">'.$this->__('Send').'</button></div><div class="notice" id="ajax-response"></div>'),
  		);
    ...
    {
        $ajaxUrl = $this->getUrl('magecomp/support');
        $html = parent::_getFooterHtml($element);
        $html = '<h4>'.$this->__('Contact Magecomp Support Team or visit <a href="%s" target="new">%s</a> for additional information', 'http://www.magecomp.com/', 'Magecomp.com').'</h4>' . $html;

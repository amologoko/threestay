...
class Apptha_Airhotels_PropertyController extends Mage_Core_Controller_Front_Action {
    ...
    public function updateAction() {
        ...
        if ($result == 1) {
            ...
            $url = Mage::getBaseUrl() . "booking/property/image/id/" . $post["id"];
        } else {
...
// if current month is Decembe or January month navigation links have to be updated to point to next / prev years
        ...
        $totaldays = date("t", $date);
        echo '<a class="pre_grid" href="javascript:void(0);" onclick="ajaxLoadCalendar(\'' . Mage::getBaseUrl() . 'booking/property/calendarview/?date=' . $prev_month . '__' . $prev_year . '&productid=' . $productId . '\')" >previous</a>';
        echo '<div class="date_grid">' . date("F, Y", $date) . '</div>';
        echo '<a class="next_grid" href="javascript:void(0);" onclick="ajaxLoadCalendar(\'' . Mage::getBaseUrl() . 'booking/property/calendarview/?date=' . $next_month . '__' . $next_year . '&productid=' . $productId . '\')" >next</a>';
